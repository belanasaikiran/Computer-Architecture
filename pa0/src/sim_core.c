...
void process_instructions() {
    ...
    while (terminate != 1) {
        ...
        committed_inst  = writeback(mem_out);
        mem_out_n       = memory_stage(ex_out);
        ex_out_n        = execute(decode_out);
        decode_out_n    = decode(fetch_out);
        fetch_out_n     = fetch(fetch_out);

        /* Increment the total instruction counter */
        if (mem_out.inst != 0x00000013){
            instruction_counter++;
        }
